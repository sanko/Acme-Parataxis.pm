...
pending, the callback fires immediately and the fiber is not suspended.

=head2 C<awaited( )>

    my $n = $sig->awaited;

Returns the number of fibers (or callbacks) currently waiting on this signal. Returns 0 when nobody is waiting.

=head2 C<remove_waiter( $waiter )>

    my $removed = $sig->remove_waiter($fid);       # by fiber id
    my $removed = $sig->remove_waiter(\&callback); # or by callback

Unregister a fiber id or callback that is currently waiting on this signal. Returns the number of entries removed (0
or 1). Consumes nothing: a remembered signal is left untouched and no callback is invoked. Removed fibers must be
reclaimed by the caller (typically by destroying them).
