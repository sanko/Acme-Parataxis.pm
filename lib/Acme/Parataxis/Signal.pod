...
Creates a new signal in the unsignalled state. You can also pass C<count-E<gt>true> to create a signal that is already
signalled (i.e., the next C<wait> returns immediately).
...
Wait for the signal. In the no-argument form, suspends the current fiber until the signal is sent. If the signal is
