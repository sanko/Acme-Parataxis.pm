...
C<with_timeout> and cancellation just like any other block. Supervision (restart-on-failure, OTP-style) is out of
scope; a handler die fails its own ask (or warns, for a C<send>) and the actor keeps going.
