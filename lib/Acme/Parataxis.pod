...
An alias for C<spawn( )>. It creates a new fiber and returns an C<Acme::Parataxis> fiber object that can be awaited
with C<await( )> or C<< ->await( ) >>, and also provides Future-style methods (C<result>, C<on_ready>).
...
Creates a new fiber and runs it. Returns an C<Acme::Parataxis> fiber object that can be awaited with C<await( )> or
C<< ->await( ) >> and will eventually contain the fiber's return value.
...
=head1 CORO-STYLE MESSAGE PASSING

Two modules mirror the Coro distribution so that code written for
L<Coro> and L<Coro::Channel> can run unchanged:

=head2 C<Acme::Parataxis::Semaphore>

A counting semaphore with the same interface as L<Coro::Semaphore>:
C<new>, C<count>, C<down>, C<up>, C<try>, C<adjust>, C<wait>, C<waiters>
and C<guard>.  Fibers block (park) while waiting for a permit and are
resumed in FIFO order as permits become available:

    use Acme::Parataxis;
    use Acme::Parataxis::Semaphore;

    my $sem = Acme::Parataxis::Semaphore->new( 2 );   # two permits

    async {
        my $t = fiber { $sem->down; say 'acquired' };
        $sem->up;
    };

=head2 C<Acme::Parataxis::Channel>

A Coro::Channel-style message queue: a bounded, thread-safe-ish pipe with
C<put>, C<get>, C<shutdown>, C<size> and C<new( $maxsize )>.  Writers
block while the queue is full and readers block while it is empty; a
size-C<1> channel is a rendezvous point.  Consumers see end-of-data once
C<shutdown> is called:
    ...
    async {
        fiber { $q->put( $_ ) for 1 .. 8 };           # producer
        say $q->get while 1;                          # consumer
    };

=head2 C<Acme::Parataxis::Signal>

A Coro::Signal-style binary semaphore / event flag: C<new>, C<wait>
(with optional callback), C<send>, C<broadcast> and C<awaited>.
C<send> wakes up I<one> waiter or remembers the signal if nobody is
waiting; C<broadcast> wakes up I<all> waiters or loses the signal;
C<wait> consumes a remembered signal or parks until one arrives.  A
callback passed to C<wait> is invoked in the sending fiber's context:
    ...
    async {
        fiber { $sig->wait; say 'go!' };
        $sig->send;
    };

B<Note:> both modules park fibers while blocked, so they behave like the
C<await_*> functions.  Avoid ceding to the scheduler (C<yield>) from
inside a C<guard> critical section: with all permits held and the holder
also waiting, no runnable fiber remains and the scheduler reports a
deadlock (as Coro would).
