...
C<run()> also accepts a named-argument form: C<run( virtual =E<gt> $bool, code =E<gt> $code )>. The only supported
option is C<virtual>, which switches every timer-based wait inside the run onto a virtual clock driven by the test --
see L</Deterministic Mock Time>.
