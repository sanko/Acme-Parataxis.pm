...
when the closure actually finishes. Beware that some 5.42.x perls (notably Strawberry MSWin32) can crash
C<threads-E<gt>create> once a C<feature 'class'> file has loaded a C<method DESTROY> on a second class block; if you
load such a module before your first C<spawn_blocking> on an affected perl, create a test case or call
C<spawn_blocking> from a short-lived process.
