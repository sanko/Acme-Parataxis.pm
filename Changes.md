...
- `Acme::Parataxis::Actor` gained the three hooks supervision needs: `spawn( ..., supervised => 1 )` makes a handler die kill the actor instead of only failing that one ask, `on_death( $cb )` fires exactly once when the actor is gone - with the killing error, or `undef` for a graceful stop, and immediately if it is already dead - and `respawn` returns a fresh, already-running actor with the same handler, mailbox size, and options. `error` reports why it died.
- `Actor->spawn( ..., name => $name )` registers an actor in a process-wide table, read back with `Acme::Parataxis->actor( $name )` or `->whereis( $name )`, in the style of Erlang's `register`. The registration is a strong reference, so a named actor outlives every caller that drops its handle and `respawn` keeps the key across restarts; a second live registration of the same name croaks. The name is released when the actor tears down. `$actor->swap( $code )` hot-swaps the handler at the next message boundary - a message already in flight finishes on the old code and every message dispatched after runs the new one - as a plain field read per dispatch, with no drain, stop, or rebuild.
...
- `Acme::Parataxis::Monitor` observes another fiber's death without owning it, like an Erlang monitor. It rides the target's existing completion slot rather than adding scheduler machinery, resolving once with `undef` for a clean end or the death error for a crash, and it fires immediately for an already-dead target. It watches fibers, futures, and actors alike, and the target is held only weakly so dropping a monitor lets it dissolve.
...
- Fiber cleanup via perl's native `defer` (experimental, v5.36+) needs no library API: a `defer` written lexically inside a fiber body fires when that fiber's scope exits - on a normal return, on a die, and on cancellation from a token or a `with_timeout` deadline - before the coroutine is reaped, and blocks run LIFO. Two footguns are documented in the pod: perl's `defer` is block-scoped, so a `defer` in a nested block or helper sub fires at that scope's exit rather than the fiber's, and it needs `use experimental 'defer'`.
- Fiber limits are now policy rather than a compile-time constant: `set_max_fibers( $count )` and `max_fibers()`. The fiber table is allocated on demand and doubles when it fills, bounded by an internal ceiling of 1048576, and the limit - 65536 by default - is enforced against the slots in use, so it can be raised or lowered at any time without disturbing fibers that already exist.
...
- A shared subroutine no longer loses its lexicals when one fiber parks inside it while another parks deeper. `CvDEPTH` is a single counter per CV, so when shallower frames left it dipped below the depth of a frame some other fiber had parked there, and the next entry landed on that parked frame's pad and wiped `$self`, `@_`, and the `my` lexicals in place. Activation now keeps a per-CV registry of parked depths, sets `CvDEPTH` to the deepest parked frame (plus the core's own convention for frames still on the resuming stack), and only cleans a landing pad that no parked frame owns, so a resume can never step on a pad another fiber is parked in. A fiber's registrations are purged when it is destroyed.
