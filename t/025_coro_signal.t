...
# Ported from Coro t/16_signal.t: signals / binary semaphores with the
# remember-vs-lost semantics of Coro::Signal (send wakes one waiter or
# remembers the signal; broadcast wakes everyone or loses it).
...
async {
    ...
    $sig->send;
    is( Acme::Parataxis::Semaphore::count($sig), 1, 'send remembers the signal when nobody is waiting' );

    fiber {
        my $g = $sig->wait;
        note 'wait consumed the remembered signal';
    };
    is( Acme::Parataxis::Semaphore::count($sig), 0, 'wait consumes a remembered signal without parking' );
    ...
    $sig->broadcast;
    is( Acme::Parataxis::Semaphore::count($sig), 0, 'broadcast never remembers the signal' );
    yield;
    ...
    is( Acme::Parataxis::Semaphore::count($sig), 1, 'send remembered again' );
    ...
    is( Acme::Parataxis::Semaphore::count($sig), 0, 'pending signal consumed by wait($cb)' );
