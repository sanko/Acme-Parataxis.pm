...
subtest 'a lone short sleep gets its own worker instead of queueing behind long sleeps' => sub {
    ...
    async {
        ...
        my @long =
            map { my $ms = $_; fiber { await_sleep($ms); push @long_done, [ $ms, ( time - $t0 ) * 1000 ] } }
            600, 400;
    ...
    cmp_ok $short_ms, '<', 300,
        sprintf 'a 10ms sleep came back in %.0fms with both workers busy (the bug queued it ~400ms)', $short_ms;
    ...
    ok $out_after >= 2,
        sprintf 'control: both long sleeps still in flight when it returned (outstanding=%d)', $out_after // -1;
    ...
    for my $d (@long_done) {
        my ( $ms, $el ) = @$d;
        cmp_ok $el, '>=', $ms * 0.9,
            sprintf 'control: the %dms sleeper really slept (%.0fms elapsed, not released early)', $ms, $el;
    }
};
...
subtest 'a whole backlog of short sleeps is covered in one go, not one worker per submit' => sub {
    ...
    async {
        $t0 = time;
        my @long = map { my $ms = $_; fiber { await_sleep($ms) } } 400, 400;
        Acme::Parataxis->yield while Acme::Parataxis::get_outstanding_jobs() < 2;

        # Submitted back to back with no idle worker anywhere: the pool must grow by the deficit, not trickle.
        my @s = map { fiber { my $u = time; await_sleep(20); push @took, ( time - $u ) * 1000 } } 1 .. 8;
        $_->await for @s;
        $_->await for @long;
    };
    ...
    cmp_ok $worst, '<', 250,
        sprintf 'slowest of the 8 took %.0fms (the bug left them queued until a 400ms sleep ended)', $worst;
};
