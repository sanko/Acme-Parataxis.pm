...
subtest 'get-ready vs put-ready: a ready case commits immediately' => sub {
    async {
        ...
        my ($c2, $v2) = Acme::Parataxis::Channel->select(
            [ $emptychan => 'get' ],
            [ $roomy     => 'put', 'jum' ],
        );
        ...
        $both_a->put('x');
        my ($c3, $v3) = Acme::Parataxis::Channel->select(
            [ $both_a => 'get' ],
            [ $both_b => 'put', 'y' ],
        );
...
subtest 'waiting get: woken by a producer put' => sub {
    ...
    async {
        ...
        $t0 = Time::HiRes::time();
        ($chosen, $val) = Acme::Parataxis::Channel->select(
            [ $ch => 'get' ],
            timeout => 500,
        );
...
subtest 'waiter lists are empty after every return' => sub {
    my $ch = Acme::Parataxis::Channel->new;
    async {
        my ($a, $b) = Acme::Parataxis::Channel->select(
            [ $ch => 'get' ],
            timeout => 5,
        );
